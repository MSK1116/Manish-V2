...
const Svg_study_time = () => {
  const cx = 185.2;
  const cy = 88.68;

  const [point, setPoint] = useState({ mx2: cx + 30, my2: cy, hx2: cx + 25, hy2: cy, x2: cx + 35, y2: cy });

  function getClockHandCoordinates(cx, cy) {
    const now = new Date();
    const minutes = now.getMinutes();
    const hours = (now.getHours() % 12) + minutes / 60; // Fractional hours for smooth hour hand
    const minAngle = (Math.PI / 30) * minutes - Math.PI / 2; // 6° per minute
    const hrAngle = (Math.PI / 6) * hours - Math.PI / 2; // 30° per hour
    const mx2 = cx + 30 * Math.cos(minAngle);
    const my2 = cy + 30 * Math.sin(minAngle);
    const hx2 = cx + 20 * Math.cos(hrAngle);
    const hy2 = cy + 20 * Math.sin(hrAngle);
    return {
      hx2,
      hy2,
      mx2,
  ...
  useEffect(() => {
    const getTime = getClockHandCoordinates(cx, cy);
    setPoint(getTime);
  }, []);
  function startCircularMotion(cx, cy, callback, speedMs = 1000) {
    let angle = 0;

    return setInterval(() => {
      const radians = (angle * Math.PI) / 180;

      const x2 = cx + 35 * Math.cos(radians);
      const y2 = cy + 35 * Math.sin(radians);

      callback({ x2, y2, angle });

      angle = (angle + 6) % 360; // 6° per second, like a second hand
    }, speedMs);
  }

  useEffect(() => {
    const interval = startCircularMotion(cx, cy, ({ x2, y2 }) => {
      setPoint((prev) => ({ ...prev, x2, y2 }));
    });
  ...
  return (
    <>
      <svg xmlns="http://www.w3.org/2000/svg" id="a" data-aos="zoom-in-down" data-aos-duration="1000" className="hidden md:block " width="865.76" height="682.89" viewBox="0 0 865.76 682.89">
        ...
        <g>
          <g>
            ...
            <g>
              ...
              <g>
                ...
                <path
                  d="M456.74,636.94c-6.94,0-12.59-5.65-12.59-12.59s5.65-12.59,12.59-12.59,12.59,5.65,12.59,12.59-5.65,12.59-12.59,12.59Zm0-24.18c-6.39,0-11.59,5.2-11.59,11.59s5.2,11.59,11.59,11.59,11.59-5.2,11.59-11.59-5.2-11.59-11.59-11.59Z"
                  fill="#303a46"></path>
              </g>
              ...
              <path
                d="M558.95,281.25c-.19,0-.37-.11-.46-.29l-2.6-5.75c-.91-2.01-1.18-4.22-.8-6.39l2.95-16.47c.05-.27,.32-.46,.58-.4,.27,.05,.45,.31,.4,.58l-2.95,16.47c-.35,1.97-.1,3.98,.72,5.8l2.6,5.75c.11,.25,0,.55-.25,.66-.07,.03-.14,.04-.21,.04h.02Z"
                fill="#303a46"></path>
              ...
              <path
                d="M443.05,354.87s-41.59-2.19-42.83,37.28c0,0-.83,13.43,8.27,45.06l22.32,96.1s-7.03,17.36,5.79,17.77c0,0,26.87,3.31,38.65-16.95l-5.99-107.47s10.13,8.27,22.94,11.57c0,0,27.69-7.65,64.48-5.58,0,0,45.26-21.91,20.87-80.81l-132.27-3.51-2.22,6.53h0Z"
                fill="#5ac1b0"></path>
              <path
                d="M405.89,428.17c-.22,0-.43-.15-.48-.38-6.28-24.6-5.7-35.57-5.69-35.68,.84-26.65,20.18-34.44,31.63-36.7,.27-.05,.53,.12,.59,.39s-.12,.53-.39,.59c-13.64,2.7-30.05,11.1-30.83,35.77,0,.12-.57,10.97,5.66,35.38,.07,.27-.09,.54-.36,.61-.04,.01-.08,.02-.12,.02h-.01Z"
                fill="#303a46"></path>
              <path
                d="M360.33,587.05h-4.55l-8.68-13.02c-4.55-8.06-10.54-7.03-10.54-7.03h-7.65c-4.2,0-4.92,2.33-4.96,2.47h0l-.87-6.25c14.21-1.7,30.22-6.77,30.22-6.77,.29,7.92,1.81,15.05,3.36,20.43,1.81,6.26,3.67,10.15,3.67,10.15v.02h0Z"
                fill="#f9b499"></path>
              <g>
              ...
              <path
                d="M574.65,467.26c-.19,0-.37-.11-.46-.3-.11-.25,0-.55,.25-.66l8.73-3.89c.47-.23,.75-.69,.75-1.22s-.29-.99-.78-1.23l-31.93-15.33c-.25-.12-.35-.42-.23-.67s.42-.35,.67-.23l31.93,15.33c.82,.4,1.34,1.21,1.34,2.13s-.51,1.73-1.33,2.13l-8.74,3.9c-.07,.03-.14,.04-.2,.04h0Z"
                fill="#303a46"></path>
              <g>
              ...
              <path
                d="M512.86,276.94c-.53,0-.97-.42-1-.95-.03-.55,.4-1.02,.95-1.05,3.46-.17,4.73-1.51,5.75-3.76,.36-.79,.61-2.12,.83-3.3,.13-.67,.25-1.31,.38-1.82,2.22-8.65,9.25-7.81,9.33-7.8,.55,.07,.93,.58,.86,1.12s-.58,.93-1.12,.86c-.22-.03-5.36-.57-7.13,6.31-.12,.48-.24,1.07-.35,1.7-.25,1.29-.52,2.75-.98,3.75-1.1,2.42-2.77,4.69-7.47,4.93h-.05Z"
                fill="#303a46"></path>
              <path
                d="M603.38,291.5c-5.79-10.33-10.45-22.92-10.45-22.92-23.54,13.45-33.98,11.96-33.98,11.96l5.46,12.75c-7.15,2.42-31.5-.35-31.5-.35-.83-1.86-3.1-2.27-3.1-2.27-1.86-1.24-1.65-3.1-1.65-3.1-.62-6.41-4.75-5.99-4.75-5.99l-12.85,.95c-.79,.06-1.55,.31-2.21,.74l-7.59,4.89c-.46,.29-.82,.71-1.04,1.21l-4.62,10.2c-1.45,5.58,5.9,2.32,5.9,2.32l-5.17,6.82c1.03,2.89,4.85,3.26,4.85,3.26,1.45,6.41,6.61,6.61,6.61,6.61,5.99,9.3,24.39,.83,24.39,.83l27.07,5.79c37.2,7.65,43.4,1.86,43.4,1.86,15.09-8.89,1.24-35.55,1.24-35.55h-.01Zm-100.24,10.11l4.21-6.61c.99-1.55,2.68-2.5,4.52-2.53l8.84-.16-17.57,9.3Z"
                fill="#f9b499"></path>
              <path
                d="M494.9,301.22h-.02c-.28-.01-.49-.24-.48-.52,.01-.38,.08-.8,.2-1.26l4.65-10.28c.26-.58,.69-1.08,1.23-1.42l1.32-.85c.23-.15,.54-.08,.69,.15,.15,.23,.08,.54-.15,.69l-1.32,.85c-.38,.24-.67,.59-.86,.99l-4.62,10.2c-.07,.31-.13,.66-.14,.97-.01,.27-.23,.48-.5,.48h0Z"
                fill="#303a46"></path>
              <path
                d="M599.04,295.72c-.13,0-.25-.05-.35-.14-11.18-10.98-24.11-4.93-24.23-4.87-7.32,3.55-10.14,2.9-10.25,2.87-.27-.07-.43-.34-.36-.61,.07-.26,.33-.43,.6-.36,.06,.01,2.72,.53,9.58-2.8,.14-.07,13.68-6.43,25.37,5.05,.2,.19,.2,.51,0,.71-.1,.1-.23,.15-.36,.15h0Z"
                fill="#303a46"></path>
              <path
                d="M532.91,318.81c-.19,0-.37-.1-.45-.28-.12-.25-.01-.55,.23-.67,1.33-.64,2.18-1.53,2.52-2.66,.65-2.13-.67-4.42-.68-4.44-.1-.17-.09-.39,.03-.56,3.02-3.96-.45-8.08-.6-8.26-.12-.14-.15-.33-.09-.51,2.28-5.89-.88-7.84-1.02-7.92-.24-.14-.32-.45-.18-.68,.14-.24,.44-.32,.68-.18,.17,.09,3.95,2.35,1.55,8.88,.76,1.02,3.4,5.07,.64,9.02,.38,.76,1.25,2.87,.62,4.94-.43,1.4-1.45,2.5-3.04,3.26-.07,.03-.14,.05-.22,.05h0Z"
                fill="#303a46"></path>
              <path
                d="M501.55,302.43c-.4,0-.64-.05-.68-.06-.27-.06-.43-.33-.37-.6,.06-.27,.33-.43,.6-.37,.01,0,.33,.07,.84,.02,.46-.05,.93-.2,1.38-.44l19.44-10.38c.24-.13,.54-.04,.68,.21,.13,.24,.04,.55-.21,.68l-19.44,10.38c-.57,.3-1.16,.49-1.75,.55-.18,.02-.34,.03-.49,.03v-.02h0Z"
                fill="#303a46"></path>
              ...
              <path
                d="M432.32,287.35l-6.48,8.9,18.19,2.48s2.69-2.89,7.65-1.03c0,0,3.31-4.34,8.06,1.45l7.29,11.99c1.18,1.94,2.54,3.76,4.07,5.43,2.43,2.67,5.96,7.09,4.28,8.77-.17,.17-.38,.28-.6,.35-1.17,.38-6.1,1.69-10.08-1.74l-6.09-5.12h0s.72,4.5-.73,6.77l-27.07,.21-2.27-3.51h-16.74l-22.73-26.04,11.58-20.95s18.95,11.82,31.69,12.05h-.02Z"
                fill="#f9b499"></path>
              <path
                d="M471.1,317.06c-.14,0-.27-.05-.37-.16-1.53-1.68-2.92-3.53-4.12-5.51l-2.81-4.62c-.14-.24-.07-.54,.17-.69,.23-.14,.54-.07,.69,.17l2.81,4.62c1.17,1.92,2.52,3.72,4.01,5.36,.19,.2,.17,.52-.03,.71-.1,.09-.22,.13-.34,.13h-.01Z"
                fill="#303a46"></path>
              ...
              <path
                d="M421.92,306.67c-.07,0-.13-.01-.2-.04-.25-.11-.37-.4-.26-.66,.03-.06,2.66-6.01,10.23-5.68,.97-.67,6.26-4.02,12.53-1.67,.26,.1,.39,.39,.29,.64-.1,.26-.39,.39-.64,.29-6.28-2.36-11.67,1.6-11.73,1.64-.09,.07-.21,.1-.33,.1-7.05-.38-9.41,5.02-9.43,5.07-.08,.19-.27,.3-.46,.3h0Z"
                fill="#303a46"></path>
            ...
            <path
              d="M621.64,549.46c-.28,0-.5-.22-.5-.5v-31.66c0-10.59-8.62-19.2-19.2-19.2s-19.21,8.62-19.21,19.2v31.33c0,.28-.22,.5-.5,.5s-.5-.22-.5-.5v-31.33c0-11.14,9.06-20.2,20.21-20.2s20.2,9.06,20.2,20.2v31.66c0,.28-.22,.5-.5,.5Z"
              fill="#303a46"></path>
            <path
              d="M246.86,623c-.23,0-.44-.16-.49-.4l-6.26-30.81c-.17-.85-.55-1.6-1.09-2.18s-1.1-.84-1.64-.8c-1.13,.1-2.01,1.43-2.02,1.44-7.76,11.77-21.16,11.41-21.29,11.4h-.46c-14.18,0-20.01-11.33-20.07-11.45-.96-1.48-2.12-2.03-3.39-1.58-.96,.34-1.68,1.2-1.88,2.26l-6.2,31.72c-.05,.27-.33,.44-.59,.4-.27-.05-.45-.32-.39-.59l6.2-31.72c.27-1.4,1.25-2.56,2.53-3.01,1.22-.43,3-.42,4.59,2.03,.08,.16,5.79,11.15,19.68,10.94,.17,.01,13,.34,20.44-10.95,.04-.07,1.15-1.74,2.76-1.89,.88-.08,1.7,.3,2.46,1.11,.67,.71,1.13,1.63,1.34,2.66l6.26,30.81c.05,.27-.12,.53-.39,.59-.03,0-.07,.01-.1,.01h0Z"
              fill="#303a46"></path>
            ...
            <g>
              ...
              <path
                d="M635.4,650.73h-47.39c-8.99,0-16.3-7.31-16.3-16.3v-87.28c0-8.99,7.31-16.3,16.3-16.3h47.39c8.99,0,16.3,7.31,16.3,16.3v87.28c0,8.99-7.31,16.3-16.3,16.3h0Zm-47.39-118.87c-8.43,0-15.3,6.86-15.3,15.3v87.28c0,8.43,6.86,15.3,15.3,15.3h47.39c8.44,0,15.3-6.86,15.3-15.3v-87.28c0-8.44-6.86-15.3-15.3-15.3h-47.39Z"
                fill="#f9d46c"></path>
            </g>
            ...
            <g>
              ...
              <path
                d="M589.69,613.7h-.16c-1.29-.06-2.37-.88-2.8-2.13-1.21-3.5,1.46-9.07,2.31-10.69,.12-.22,.35-.36,.6-.37h0c.25,0,.49,.14,.61,.36,2.62,4.82,3.39,8.51,2.27,10.96-.53,1.16-1.61,1.87-2.84,1.87h.01Zm-.02-11.82c-.88,1.79-2.96,6.52-1.98,9.37,.35,1.02,1.21,1.42,1.9,1.46,.71,.04,1.59-.29,2.04-1.28,.65-1.43,.82-4.25-1.96-9.54h0Z"
                fill="#5ac1b0"></path>
            </g>
            <path
              d="M632.14,556.89c-.28,0-.5-.22-.5-.5v-31.66c0-10.59-8.62-19.2-19.21-19.2s-19.21,8.62-19.21,19.2v31.33c0,.28-.22,.5-.5,.5s-.5-.22-.5-.5v-31.33c0-11.14,9.06-20.2,20.21-20.2s20.21,9.06,20.21,20.2v31.66c0,.28-.22,.5-.5,.5Z"
              fill="#303a46"></path>
            <g>
            ...
            <path
              d="M218.75,546.75c-.26,0-.47-.2-.5-.46-.04-.41-3.04-40.71,32.66-52.86,.26-.1,7.05-2.73,14.4,.65,5.84,2.68,10.3,8.35,13.24,16.84,.09,.26-.05,.55-.31,.64-.26,.09-.54-.05-.64-.31-2.85-8.22-7.12-13.69-12.7-16.25-6.96-3.2-13.59-.65-13.66-.62-35,11.91-32.05,51.44-32.01,51.84,.02,.28-.18,.52-.46,.54h-.02Z"
              fill="#303a46"></path>
            ...
            <path
              d="M214.84,555.23h-.04c-.28-.02-.48-.26-.46-.54,3.67-46.48-8.03-73.47-8.15-73.73-8.67-19.25-18.82-30.23-30.18-32.64-11.57-2.45-20.27,5.19-20.36,5.27-.21,.18-.52,.17-.71-.04-.18-.21-.17-.52,.04-.71,.09-.08,9.17-8.06,21.23-5.5,11.69,2.48,22.08,13.65,30.89,33.21,.12,.27,11.93,27.47,8.23,74.22-.02,.26-.24,.46-.5,.46h.01Z"
              fill="#303a46"></path>
            ...
            <path
              d="M210.72,555.05h-.02c-.28-.01-.49-.25-.47-.52,.59-12.12-2.78-23.87-2.81-23.99-7-25.35-28.53-22.51-28.74-22.48-.27,.04-.53-.15-.57-.42-.04-.27,.15-.53,.42-.57,.22-.03,22.61-2.99,29.85,23.2,.03,.11,3.44,12.03,2.85,24.31,0,.27-.23,.48-.5,.48h0Z"
              fill="#303a46"></path>
          </g>
          <defs>
            <motion.linearGradient
              ...
              animate={{
                gradientTransform: ["translate(0, 0)", "translate(0, 1)", "translate(0, 0)"],
              }}
              transition={{
                duration: 0.5,
                repeat: Infinity,
              ...
              <stop offset="0%" stopColor="#E0A289" />
              <stop offset="100%" stopColor="f85606" />
            </motion.linearGradient>
          </defs>
          <ellipse cx="439.56" cy="145.01" rx="2.17" ry="3.2" fill="url(#fadeGradient)"></ellipse>

          <ellipse cx="472.01" cy="146.87" rx="2.17" ry="3.2" fill="url(#fadeGradient)"></ellipse>
          ...
          <path
            d="M455.79,161.94h-.11l-5.37-1.24c-.13-.03-.24-.11-.31-.23-.07-.11-.09-.25-.06-.38l3.93-15.5c.07-.27,.33-.43,.61-.36,.27,.07,.43,.34,.36,.61l-3.8,15.01,4.88,1.12c.27,.06,.44,.33,.38,.6-.05,.23-.26,.39-.49,.39h-.02v-.02Z"
            fill="#303a46"></path>
        ...
        <g>
          ...
          <g>
            ...
            <path
              d="M231.9,145.2h-94.09c-5.11,0-9.26-4.15-9.26-9.26V41.42c0-5.11,4.15-9.26,9.26-9.26h94.09c5.11,0,9.26,4.15,9.26,9.26v94.53c0,5.11-4.15,9.26-9.26,9.26h0ZM137.81,33.16c-4.55,0-8.26,3.71-8.26,8.26v94.53c0,4.55,3.71,8.26,8.26,8.26h94.09c4.55,0,8.26-3.71,8.26-8.26V41.42c0-4.55-3.71-8.26-8.26-8.26,0,0-94.09,0-94.09,0Z"
              fill="#303a46"></path>
          </g>
          <path
            d="M185.2,131.82c-23.89,0-43.32-19.35-43.32-43.14s19.43-43.14,43.32-43.14,43.32,19.35,43.32,43.14-19.43,43.14-43.32,43.14h0Zm0-85.29c-23.34,0-42.32,18.91-42.32,42.14s18.98,42.14,42.32,42.14,42.32-18.91,42.32-42.14-18.98-42.14-42.32-42.14h0Z"
            fill="#303a46"></path>
          {/* <path
            d="M185.2,89.18c-.11,0-.22-.04-.31-.11l-24.22-19.03c-.22-.17-.25-.48-.08-.7,.17-.22,.49-.25,.7-.08l22.98,18.05-4.81-16.11c-.08-.26,.07-.54,.34-.62,.27-.08,.54,.07,.62,.34l5.26,17.62c.06,.21-.02,.43-.2,.56-.08,.06-.18,.09-.28,.09h0Z"
            fill="#303a46"></path> */}
          <line x1="185.2" y1="88.68" x2={point.mx2} y2={point.my2} stroke="#303a46" strokeWidth="1" strokeLinecap="round"></line>
          <line x1="185.2" y1="88.68" x2={point.hx2} y2={point.hy2} stroke="#303a46" strokeWidth="1" strokeLinecap="round"></line>
          <line x1="185.2" y1="88.68" x2={point.x2} y2={point.y2} stroke="#303a46" strokeWidth="1" strokeLinecap="round"></line>
        </g>
        <g id="b">
          <g opacity=".1">
          ...
          <g>
            ...
            <path
              d="M741.41,222.55h-91.49c-.15,0-.29-.07-.39-.18-.09-.11-.13-.27-.11-.41l2.8-14.56c5.95-35.47,7.45-51.6,6.63-97.48l-.95-18.78c0-.14,.04-.27,.14-.37,.09-.1,.23-.16,.36-.16h86.78c.26,0,.48,.21,.5,.47l1.02,17.75c2.18,76.44-4.74,112.94-4.81,113.3-.05,.23-.25,.4-.49,.4v.02h.01Zm-90.88-1h90.47c.74-4.18,6.77-40.66,4.72-112.66l-1-17.27h-85.79l.92,18.27c.82,45.97-.68,62.13-6.64,97.69l-2.69,13.98h.01Z"
              fill="#303a46"></path>
          </g>
          <path
            d="M663.55,97.72c-1.3-.11-2.28-1.25-2.17-2.55,.09-1.16,1.01-2.08,2.17-2.17,.19,0,.35,.16,.35,.35h0c0,.19-.16,.35-.35,.35h0c-.92,0-1.67,.75-1.67,1.67s.75,1.67,1.67,1.67,1.67-.75,1.67-1.67h0c0-.19,.16-.35,.35-.35h0c.19,0,.35,.16,.35,.35h0c0,1.3-1.07,2.36-2.37,2.36h0Z"
            fill="#303a46"></path>
          <path
            d="M663.5,96.74c-2.17,0-2.28-4.59-2.28-5.6h0c0-.93,.11-5.63,2.28-5.63s2.31,5.06,2.32,5.64h-1.88c0-1.09-.16-2.17-.46-3.21-.27,1.03-.4,2.1-.38,3.17h0c-.03,1.07,.11,2.15,.4,3.18v-.23l.15-.5,1.3,.38c.27,.08,.43,.37,.35,.64h0c-.32,1.44-.95,2.16-1.8,2.16Z"
            fill="#303a46"></path>
          ...
          <path
            d="M674.68,97.72c-1.3-.11-2.28-1.25-2.17-2.55,.09-1.16,1.01-2.08,2.17-2.17,.19,0,.35,.16,.35,.35h0c0,.19-.16,.35-.35,.35h0c-.92,0-1.67,.75-1.67,1.67s.75,1.67,1.67,1.67,1.67-.75,1.67-1.67h0c0-.19,.16-.35,.35-.35h0c.19,0,.35,.16,.35,.35h0c0,1.3-1.07,2.36-2.37,2.36h0Z"
            fill="#303a46"></path>
          <path
            d="M674.63,96.74c-2.16,0-2.28-4.59-2.28-5.6h0c0-.93,.11-5.63,2.28-5.63s2.26,5.03,2.26,5.61h-1.88c0-1.09-.16-2.17-.46-3.21-.27,1.03-.4,2.1-.38,3.17h0c-.03,1.07,.11,2.15,.4,3.18v-.23l.15-.5,1.3,.38c.27,.08,.43,.37,.35,.64h0c-.26,1.47-.89,2.19-1.74,2.19Z"
            fill="#303a46"></path>
          ...
          <path
            d="M685.81,97.72c-1.3-.11-2.28-1.25-2.17-2.55,.09-1.16,1.01-2.08,2.17-2.17,.19,0,.35,.16,.35,.35h0c0,.19-.16,.35-.35,.35h0c-.92,0-1.67,.75-1.67,1.67s.75,1.67,1.67,1.67,1.67-.75,1.67-1.67h0c0-.19,.16-.35,.35-.35h0c.19,0,.35,.16,.35,.35h0c0,1.3-1.07,2.36-2.37,2.36h0Z"
            fill="#303a46"></path>
          <path
            d="M685.77,96.74c-2.16,0-2.28-4.59-2.28-5.6h0c0-.93,.11-5.63,2.28-5.63s2.31,5.06,2.32,5.64h-1.88c0-1.09-.16-2.17-.46-3.21-.27,1.03-.4,2.1-.38,3.17h0c-.03,1.07,.11,2.15,.4,3.18v-.23l.15-.5,1.3,.38c.27,.08,.43,.37,.35,.64h0c-.32,1.44-.95,2.16-1.8,2.16Z"
            fill="#303a46"></path>
          ...
          <path
            d="M696.95,97.72c-1.3-.11-2.28-1.25-2.17-2.55,.09-1.16,1.01-2.08,2.17-2.17,.19,0,.35,.16,.35,.35h0c0,.19-.16,.35-.35,.35h0c-.92,0-1.67,.75-1.67,1.67s.75,1.67,1.67,1.67,1.67-.75,1.67-1.67h0c0-.19,.16-.35,.35-.35h0c.19,0,.35,.16,.35,.35h0c0,1.3-1.05,2.36-2.35,2.36h-.02Z"
            fill="#303a46"></path>
          <path
            d="M696.9,96.74c-2.16,0-2.28-4.59-2.28-5.6h0c0-.93,.11-5.63,2.28-5.63s2.31,5.06,2.32,5.64h-1.88c0-1.09-.16-2.17-.46-3.21-.27,1.03-.4,2.1-.38,3.17h0c-.03,1.07,.11,2.15,.4,3.18v-.23l.15-.5,1.3,.38c.27,.08,.43,.37,.35,.64h0c-.32,1.44-.95,2.16-1.8,2.16Z"
            fill="#303a46"></path>
          ...
          <path
            d="M708.02,97.72c-1.3,.11-2.45-.87-2.55-2.17-.11-1.3,.87-2.45,2.17-2.55,.13-.01,.25-.01,.38,0,.19,0,.35,.16,.35,.35h0c0,.19-.16,.35-.35,.35h0c-.92,0-1.67,.73-1.68,1.65s.73,1.67,1.65,1.68,1.67-.73,1.68-1.65v-.02c0-.19,.16-.35,.35-.35h0c.19,0,.35,.16,.35,.35h0c0,1.3-1.05,2.36-2.36,2.36h.01Z"
            fill="#303a46"></path>
          <path
            d="M707.96,96.74c-2.16,0-2.28-4.59-2.28-5.6h0c0-.93,.11-5.63,2.28-5.63s2.32,5.06,2.34,5.64h-1.88c0-1.09-.16-2.17-.47-3.21-.27,1.03-.4,2.1-.38,3.17h0c-.02,1.07,.11,2.15,.4,3.18,.03-.07,.06-.15,.08-.23l.14-.5,1.32,.38c.27,.08,.43,.37,.35,.64h0c-.38,1.44-1.02,2.16-1.88,2.16h-.02Z"
            fill="#303a46"></path>
          ...
          <path
            d="M719.15,97.72c-1.3,.11-2.45-.87-2.55-2.17-.11-1.3,.87-2.45,2.17-2.55,.13-.01,.25-.01,.38,0,.19,0,.35,.16,.35,.35h0c0,.19-.16,.35-.35,.35h0c-.92,0-1.67,.73-1.68,1.65s.73,1.67,1.65,1.68,1.67-.73,1.68-1.65v-.02c0-.19,.16-.35,.35-.35h0c.19,0,.35,.16,.35,.35h0c0,1.3-1.05,2.36-2.36,2.36h.01Z"
            fill="#303a46"></path>
          <path
            d="M719.09,96.74c-2.16,0-2.28-4.59-2.28-5.6h0c0-.93,.11-5.63,2.28-5.63s2.32,5.06,2.34,5.64h-1.88c0-1.09-.16-2.17-.47-3.21-.27,1.03-.4,2.1-.38,3.17h0c-.02,1.07,.11,2.15,.4,3.18,.03-.07,.06-.15,.08-.23l.14-.5,1.32,.38c.27,.08,.43,.37,.35,.64h0c-.38,1.44-1.02,2.16-1.88,2.16h-.02Z"
            fill="#303a46"></path>
          ...
          <path
            d="M730.28,97.72c-1.3,.11-2.45-.87-2.55-2.17-.11-1.3,.87-2.45,2.17-2.55,.13-.01,.25-.01,.38,0,.19,0,.35,.16,.35,.35h0c0,.19-.16,.35-.35,.35h0c-.92,0-1.67,.75-1.67,1.67s.75,1.67,1.67,1.67,1.67-.75,1.67-1.67h0c0-.19,.16-.35,.35-.35h0c.19,0,.35,.16,.35,.35h0c0,1.3-1.05,2.36-2.35,2.36h-.02Z"
            fill="#263238"></path>
          <path
            d="M730.22,96.74c-2.16,0-2.28-4.59-2.28-5.6h0c0-.93,.11-5.63,2.28-5.63s2.32,5.06,2.34,5.64h-1.88c0-1.09-.16-2.17-.47-3.21-.27,1.03-.4,2.1-.38,3.17h0c-.02,1.07,.11,2.15,.4,3.18,.03-.07,.06-.15,.08-.23l.14-.5,1.32,.38c.27,.08,.43,.37,.35,.64h0c-.38,1.44-1.02,2.16-1.88,2.16h-.02Z"
            fill="#303a46"></path>
          ...
          <path
            d="M741.41,97.72c-1.3-.11-2.28-1.25-2.17-2.55,.09-1.16,1.01-2.08,2.17-2.17,.19,0,.35,.16,.35,.35h0c0,.19-.16,.35-.35,.35h0c-.92,0-1.67,.73-1.68,1.65s.73,1.67,1.65,1.68,1.67-.73,1.68-1.65v-.02c0-.19,.16-.35,.35-.35h0c.19,0,.35,.16,.35,.35h0c0,1.3-1.05,2.36-2.36,2.36h.01Z"
            fill="#263238"></path>
          <path
            d="M741.35,96.74c-2.16,0-2.28-4.59-2.28-5.6h0c0-.93,.12-5.63,2.28-5.63s2.32,5.06,2.34,5.64h-1.88c0-1.09-.16-2.17-.47-3.21-.27,1.03-.4,2.1-.38,3.17h0c-.02,1.07,.11,2.15,.4,3.18,.03-.07,.06-.15,.08-.23l.14-.5,1.32,.38c.27,.08,.43,.37,.35,.64h0c-.38,1.44-1.02,2.16-1.88,2.16h-.02Z"
            fill="#303a46"></path>
          ...
          <g>
            ...
            <path
              d="M719.07,88.45h-13.4c-.28,0-.5-.22-.5-.5v-9.4c0-1.48-1.2-2.68-2.68-2.68s-2.68,1.2-2.68,2.68v9.4c0,.28-.22,.5-.5,.5h-13.56c-.28,0-.5-.22-.5-.5s.22-.5,.5-.5h13.06v-8.9c0-2.03,1.65-3.68,3.68-3.68s3.68,1.65,3.68,3.68v8.9h12.9c.28,0,.5,.22,.5,.5s-.22,.5-.5,.5Z"
              fill="#303a46"></path>
          </g>
          ...
          <path
            d="M696.12,197.56c-.27,0-.49-.21-.5-.48-.02-.62,.04-1.24,.16-1.85,.18-.49,.29-.92,.35-1.35,.04-.27,.29-.47,.56-.43,.27,.04,.47,.29,.43,.56-.07,.52-.2,1.02-.38,1.51-.09,.45-.14,1-.12,1.53,0,.28-.21,.51-.48,.52h-.02Z"
            fill="#303a46"></path>
          <path d="M697.74,192.68c-.28,0-.5-.22-.5-.5v-.12c0-.28,.22-.5,.5-.5s.5,.22,.5,.5v.12c0,.28-.22,.5-.5,.5Z" fill="#303a46"></path>
          <path
            d="M700.62,198.46c-.1,0-.2-.03-.29-.09-.7-.49-.5-1.46-.38-2.04,.02-.1,.04-.19,.05-.26,.17-.61,.3-1.22,.39-1.81-.38,.39-.7,.83-.94,1.32-.23,.47-.47,.96-.69,1.46-.05,.21-.15,.41-.29,.57-.14,.16-.38,.21-.57,.12-.2-.09-.32-.29-.29-.51,.06-.57,.21-1.05,.36-1.51,.1-.31,.2-.63,.26-.96,.03-.16,.04-.27,.05-.35-.13-.06-.22-.18-.26-.33-.06-.27,.1-.54,.37-.6,.25-.06,.48,0,.65,.16,.12,.12,.19,.26,.22,.42,.2-.24,.41-.47,.65-.69,.25-.38,.68-.59,1.14-.54,.27,.03,.46,.26,.45,.53-.06,1-.23,1.99-.51,2.95,0,.03-.02,.13-.05,.25-.06,.27-.19,.91-.02,1.02,.23,.16,.28,.47,.12,.7-.1,.14-.25,.21-.41,.21v-.02h-.01Z"
            fill="#303a46"></path>
          <path
            d="M702.16,198.34c-.07,0-.14,0-.22-.04-.23-.08-.37-.3-.34-.54,.19-1.41,.54-2.79,1.04-4.12,.1-.26,.38-.39,.64-.29s.39,.39,.29,.64c-.32,.83-.57,1.7-.75,2.57,.06,0,.11,.03,.16,.06,.24,.14,.33,.44,.19,.68-.17,.35-.5,1.03-1.02,1.03h.01Z"
            fill="#303a46"></path>
          <path d="M704.24,192.44c-.28,0-.5-.22-.5-.5v-.12c0-.28,.22-.5,.5-.5s.5,.22,.5,.5v.12c0,.28-.22,.5-.5,.5Z" fill="#303a46"></path>
          <path
            d="M704.77,198.95c-.08,0-.17,0-.25-.04-.18-.07-.48-.26-.48-.83,0-.28,.22-.5,.5-.5,.19,0,.35,.1,.44,.25,.22-.15,.51-.41,.72-.65,.23-.27,.33-.47,.28-.6-.15-.13-.35-.21-.54-.19h-.26c-.29-.02-1.17-.08-1.17-.88,0-.1,.03-.2,.09-.28l.34-.49s.05-.06,.08-.09l.63-.57c.54-.45,1.12-.84,1.71-1.17,.24-.13,.54-.05,.68,.2,.13,.24,.04,.55-.2,.68-.54,.3-1.06,.65-1.54,1.05l-.59,.54h.05c.07,0,.14,0,.19,.02,.5,.02,.95,.19,1.29,.53,.02,.02,.05,.05,.06,.08,.24,.38,.38,1-.35,1.84-.24,.28-1.02,1.11-1.68,1.11h0Z"
            fill="#303a46"></path>
          <path
            d="M712.9,198.56h-.05c-.28-.03-.48-.27-.45-.55,.02-.18,.04-.37,.08-.55-.04,.02-.07,.04-.11,.05-1.03,.33-1.81-.35-2.19-1.03-.17-.36-.3-.71-.42-1.07-.12,.12-.27,.3-.37,.41-.08,.1-.16,.19-.24,.28-.21,.23-.4,.48-.59,.72-.16,.21-.32,.42-.5,.63-.16,.19-.44,.23-.64,.1-.5-.31-.3-1,.25-2.38l.08-.2c.21-.59,.44-1.17,.68-1.75,.11-.28,.23-.56,.34-.84,.1-.26,.39-.38,.65-.28s.38,.39,.28,.65c-.11,.28-.23,.57-.34,.85-.19,.46-.37,.91-.54,1.38,.37-.42,.85-.85,1.46-.63,.15,.05,.27,.18,.31,.33,.13,.46,.3,.92,.5,1.35,.02,.04,.42,.71,.92,.55,.37-.18,.7-.45,.97-.78,.03-.08,.07-.15,.1-.22,.11-.41,.34-.86,.66-1.25,.15-.18,.41-.23,.61-.12,.21,.11,.31,.35,.25,.57-.15,.56-.41,1.09-.76,1.54-.24,.61-.39,1.19-.45,1.8-.03,.26-.24,.45-.5,.45h.01Zm-4.77-1.61h0Z"
            fill="#303a46"></path>
